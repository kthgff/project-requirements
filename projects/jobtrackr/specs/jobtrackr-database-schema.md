...
- allow editing and job management without manual job creation in MVP
...
CREATE INDEX idx_gmail_connections_last_synced_at ON gmail_connections(last_synced_at);
```
...
Indexes:
```sql
CREATE INDEX idx_source_emails_user_id ON source_emails(user_id);
CREATE INDEX idx_source_emails_thread_id ON source_emails(gmail_thread_id);
CREATE INDEX idx_source_emails_parse_status ON source_emails(parse_status);
CREATE INDEX idx_source_emails_internal_date ON source_emails(internal_date DESC);
CREATE INDEX idx_source_emails_classification ON source_emails(classification);
```

Notes:
- message_hash may help secondary dedupe or debugging
- `classification` stores canonical source-email outcomes such as `job_alert`, `uncertain`, or `irrelevant`
...
- fit fields remain nullable until analysis runs
- parse_confidence is optional but useful for future tuning and review workflows

## 6. job_source_emails
Join table preserving one-source-email-to-many-jobs linkage.

```sql
CREATE TABLE job_source_emails (
  job_id UUID NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  source_email_id UUID NOT NULL REFERENCES source_emails(id) ON DELETE CASCADE,
  linkage_type TEXT NOT NULL DEFAULT 'candidate_origin',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  PRIMARY KEY (job_id, source_email_id)
);
```
